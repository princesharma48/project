...
//         // If product images saved successfully
//         if ($cat_img_run) {
//             // Saving tab data
//             $tabs_data = array();
//             if (isset($_POST['tabs'])) {
//                 foreach ($_POST['tabs'] as $tab) {
//                     $tabs_data[$tab] = mysqli_real_escape_string($conn, $_POST[strtolower($tab)]);
//                     $query_tab = "INSERT INTO $tab (product_id, content) VALUES ('$last_id', '$tabs_data[$tab]')";
//                     $query_tab_run = mysqli_query($conn, $query_tab);

//                     if (!$query_tab_run) {
//                         $_SESSION['error'] = "Failed to save data in $tab tab.";
...
//             exit;
//         } else {
//             $_SESSION['error'] = "Failed to save product images.";
//             header("Location: products.php");
//             exit;
//         }
//     } else {
...
// }
if (isset($_POST['save_category'])) {
    // Ensure no output is sent before header() is called
    ob_start();
    ...
    if ($query_run) {
        ...
        if ($cat_img_run) {
            ...
            if (isset($_POST['tabs']) && is_array($_POST['tabs'])) { // Check if tabs data is an array
                foreach ($_POST['tabs'] as $tab) {
                    $tabs_data[$tab] = mysqli_real_escape_string($conn, $_POST[strtolower($tab)]);
                    $tab_table_name = $tab . "_tab"; // Assuming you have tab tables with names like 'features_tab', 'specification_tab', 'gallery_tab'
                    $query_tab = "INSERT INTO $tab_table_name (product_id, content) VALUES ('$last_id', '$tabs_data[$tab]')";
                    $query_tab_run = mysqli_query($conn, $query_tab);
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        function addTextEditor(tabName) {
            ...
            editorContainer.innerHTML = `
                ...
                <textarea id="${editorId}" name="tabs" class="form-control"></textarea>
            `;
